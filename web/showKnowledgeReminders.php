...
$sql = "SELECT p.Topic_ID, p.PostNo, p.content, p.user_email, p.title, p.date_updated,p.topic
    FROM Posts p JOIN Posts_Updates u
 ON p.Topic_ID = u.Topic_ID AND p.PostNo = u.Postnum WHERE u.user_email = ? LIMIT ? OFFSET ?";
...
if (!$stmt) {
    echo "Error preparing statement: " . $conn->error;
		...
		$sql2 = "SELECT COUNT(*) AS totalKnowledgeReminders FROM Posts p JOIN Posts_Updates u
                ON p.Topic_ID = u.Topic_ID AND p.PostNo = u.Postnum WHERE u.user_email = ?";
		...
		$totalKnowledgeReminders = $result2->fetch_assoc()['totalKnowledgeReminders'];
		$stmt2->close();
		...
		echo json_encode(['posts' => $postsArray, 'totalKnowledgeReminders' => $totalKnowledgeReminders]);
