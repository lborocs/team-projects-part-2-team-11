...
$sql = "SELECT * FROM Favourites WHERE user_email = ? AND Topic_ID = ? AND PostNo = ?";
...
if ($result->num_rows > 0) {
    $sql2 = "DELETE FROM Favourites WHERE user_email = ? AND Topic_ID = ? AND PostNo = ?";
    ...
    $sql2 = "INSERT INTO Favourites (user_email, Topic_ID, PostNo) VALUES (?, ?, ?)";
    $stmt2 = $conn->prepare($sql2);
    if (!$stmt2) {
