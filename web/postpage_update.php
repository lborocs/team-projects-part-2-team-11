...
<script>
    ...
    $.ajax({
            ...
            type: "GET",
            //sessions needed here
            data: {email: "olivia.rodriguez@makeitall.org.uk"},
            dataType: 'json',
            success: function(response) {
    ...
    function isUpdate(element) {
        ...
        //sessions needed here
        let email = "olivia.rodriguez@makeitall.org.uk";// change this to the sessions 
        let parts = element.id.split("-");
        let topic_ID = parts[0];
        let postnum = parts[1];
        //.log(topic_ID);
	...
	$(document).ready(function() {
		...
		$.ajax({
		    ...
		    dataType: 'json',
			    ...
			    createPostElement(id, title, content,uemail,updates,date,i)
               		    }
                    },
                    error: function (e) {
...
function fetchAndUpdatePosts() {
    ...
    $.ajax({
        ...
        success: function(postsData) {
            ...
            for (let i = 0; i < postsData.length; i++) {
                // Call 'createPostElement' for each post
                createPostElement(postsData[i].post_ID, postsData[i].title, postsData[i].content,postsData[i].date_updated ,postsData[i].user_email,postsData[i].update_count, i);
            }
        },
        error: function (e) {
...
function generateTopicId(topicName) {
		...
		let title = $("#postTitle").val();
    			...
    			error: function(xhr, status, error) {
        			console.log("Error response:", xhr.responseText);
                        ...
                        alert("ERROR: You do not have permission to create post. Contact Manager for further information.");
                        closePopup()
                    }
    ...
    <script>
        $(document).ready(function () {
            ...
            let newHref = "postpage.php?category=" + encodedCategory + "&topicname=" + encodedTopic;
            ...
            if (category === "tech") {
                ...
                $("#categoryBreadcrumbs-item a").attr("href", "technical_update.php");
                console.log(0);
                    ...
                    <div id="search-container">
                        <form id="searchForm">
                            <div id="searchContent">
                                ...
                                <i class="fa fa-search search-icon" style="color: #ed992b;"></i>
                                </button>
                            </div>
                        </form>
                    </div>
